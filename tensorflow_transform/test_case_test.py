...
class TftUnitTest(test_case.TransformTestCase):
    def testCrossNamedParameters(self):
        ...
        self.assertEqual(
            test_case.cross_named_parameters(test_cases_1, test_cases_2), expected_cross
        )
    ...
    def testAssertDataCloseOrEqual(self):
        self.assertDataCloseOrEqual(
            [{"a": "first", "b": 1.0, "c": 5, "d": ("second", 2.0)}, {"e": 2, "f": 3}],
            [
                {"a": "first", "b": 1.0000001, "c": 5, "d": ("second", 2.0000001)},
                {"e": 2, "f": 3},
            ],
        )
