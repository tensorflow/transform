...
def batched_placeholders_from_specs(specs):
    ...
    """
    if not (
        all([_is_feature_spec(s) for s in specs.values()])
        or all([isinstance(s, tf.TypeSpec) for s in specs.values()])
    ):
        raise TypeError(
            "Specs must be all tf.TypeSpecs or feature specs. "
    ...
    for name, spec in specs.items():
        ...
        if spec_dtype not in (tf.int64, tf.float32, tf.string):
            raise ValueError(
                "Feature {} ({}, {}) had invalid dtype".format(name, spec, type(spec))
            )
...
def _batched_placeholder_from_typespec(name, typespec):
    ...
    if isinstance(typespec, (tf.TensorSpec, tf.SparseTensorSpec, tf.RaggedTensorSpec)):
        ...
        with tf.name_scope(sanitized_name):
            return tf.nest.map_structure(
                lambda tspec: tf.raw_ops.Placeholder(  # pylint: disable=g-long-lambda
                    dtype=tspec.dtype, shape=tspec.shape, name=sanitized_name
                ),
                typespec,
    ...
    raise ValueError(
        "Unsupported typespec: {}({}) for feature {}".format(
...
def _batched_placeholder_from_feature_spec(name, feature_spec):
    ...
    if isinstance(feature_spec, tf.io.FixedLenFeature):
        ...
        shape = (
            [None] + feature_spec.size
            if isinstance(feature_spec.size, list)
            else [None, feature_spec.size]
        )
        return tf.compat.v1.sparse_placeholder(
    ...
    raise ValueError(
        "Unsupported feature spec: {}({}) for feature {}".format(
...
def get_type_specs_from_feature_specs(
    ...
    for name, feature_spec in feature_specs.items():
        if isinstance(feature_spec, tf.io.FixedLenFeature):
            result[name] = tf.TensorSpec(
                [None] + list(feature_spec.shape), feature_spec.dtype
            )
        elif isinstance(feature_spec, tf.io.VarLenFeature):
            ...
            for partition in feature_spec.partitions:
                if isinstance(
                    partition, tf.io.RaggedFeature.UniformRowLength
...
def make_tensor_to_arrow_converter(
    ...
    options = tensor_to_arrow.TensorsToRecordBatchConverter.Options(
        sparse_tensor_value_column_name_template=schema_inference.SPARSE_VALUES_NAME_TEMPLATE,
        sparse_tensor_index_column_name_template=schema_inference.SPARSE_INDICES_NAME_TEMPLATE,
...
def get_traced_transform_fn(
    ...
    def transform_fn(inputs):
        ...
        if output_keys_to_name_map is None or not graph.get_collection(
...
def _trace_preprocessing_fn_v2(preprocessing_fn, specs, base_temp_dir):
    ...
    with annotators.object_tracker_scope(annotators.ObjectTracker()):
        concrete_fn = get_traced_transform_fn(
            preprocessing_fn, specs, tf_graph_context
...
def trace_preprocessing_function(
    preprocessing_fn, input_specs, use_tf_compat_v1, base_temp_dir=None
):
    ...
    if use_tf_compat_v1:
        ...
        return _trace_preprocessing_fn_v2(preprocessing_fn, input_specs, base_temp_dir)
...
def trace_and_write_v2_saved_model(
    ...
    _validate_analyzers_fingerprint(
        baseline_analyzers_fingerprint, concrete_transform_fn.graph, structured_inputs
    )
    ...
    if not concrete_transform_fn.graph.get_collection(
        ...
        metadata = _trace_and_get_metadata(
...
def analyze_in_place(
    ...
    transform_fn_path = os.path.join(
        transform_output_path, TFTransformOutput.TRANSFORM_FN_DIR
    )
    if use_tf_compat_v1:
        graph, structured_inputs, structured_outputs = trace_preprocessing_function(
