...
class _FixedLenFeatureHandler:
    ...
    def encode_value(self, string_list, values):
        ...
        if len(flattened_values) != self._size:
            raise ValueError(
                f'FixedLenFeature "{self._name}" got wrong number of values. Expected {self._size} but '
                f"got {len(flattened_values)}"
            )
...
class CsvCoder:
    ...
    def __init__(
        ...
        if secondary_delimiter:
            ...
            raise ValueError(
                f'secondary_delimiter unspecified for multivalent columns "{multivalent_columns}"'
            )
        ...
        for name, feature_spec in schema_utils.schema_as_feature_spec(
            ...
            if isinstance(feature_spec, tf.io.FixedLenFeature):
                ...
                raise ValueError(
                    ...
                    f"tf.VarLenFeature or tf.SparseFeature: {name!r} was {type(feature_spec)!r}"
                )
