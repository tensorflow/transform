        ...
        for current_parameters in parameters_list[1:]:
            for name, value in current_parameters.items():
                if name == "testcase_name":
                    crossed_parameters[name] = f"{crossed_parameters[name]}_{value}"
                else:
...
def parameters(*testcases):
    ...
    def wrapper(fn):
        ...
        if arg_names[0] != "self":
            raise ValueError(
                f'First argument to test is expected to be "self", but is {arg_names[0]}'
            )
        ...
        def to_arg_dict(testcase):
            ...
            if len(testcase) != len(arg_names):
                raise ValueError(
                    ...
                    f"number of expected arguments: {len(testcase)} != {len(arg_names)}, arguments: {testcase}, names: {arg_names}"
                )
