...
class AnalyzerImplsTest(tft_unit.TransformTestCase):
  ...
  def testMergeOutputsByKey(self):
    ...
    merged_outputs = analyzer_impls._merge_outputs_by_key(outputs)
    self.assertEqual(len(merged_outputs), 3)
    self.assertAllEqual(merged_outputs[0], np.array(['my_key', 'my_other_key']))
    self.assertAllEqual(merged_outputs[1], np.array([20, 23]))
    self.assertAllEqual(merged_outputs[2], np.array([[21, 22], [24, 25]]))
