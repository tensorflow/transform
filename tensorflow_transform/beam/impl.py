...
from apache_beam.typehints import Any, Dict, Iterable, List, Optional, Set, Tuple, Union
...
from tfx_bsl.tfxio import tensor_representation_util, tensor_to_arrow, tf_example_record
from tfx_bsl.tfxio.tensor_adapter import TensorAdapter, TensorAdapterConfig
...
def _clear_shared_state_after_barrier(pipeline, input_barrier):
    ...
    """
    empty_pcoll = input_barrier | "MakeCheapBarrier" >> beam.FlatMap(lambda x: None)
    return (
        pipeline
        | "PrepareToClearSharedKeepAlives" >> beam.Create([None])
        | "WaitAndClearSharedKeepAlives"
        >> beam.Map(
...
class _RunMetaGraphDoFn(beam.DoFn):
    ...
    class _GraphStateCompatV1(_GraphStateCommon):
        ...
        def __init__(
            self, saved_model_dir, input_tensor_names, exclude_outputs, tf_config
        ):
            with tf.compat.v1.Graph().as_default() as graph:
                ...
                if set(exclude_outputs).difference(outputs.keys()):
                    raise ValueError(
                        "Excluded outputs contained keys not in graph: %s"
                        % exclude_outputs
                    )
    ...
    class _GraphStateV2(_GraphStateCommon):
        ...
        def __init__(self, saved_model_dir, input_tensor_names, exclude_outputs):
            saved_model_loader = saved_transform_io_v2.SavedModelLoader(saved_model_dir)
            callable_get_outputs = saved_model_loader.apply_transform_model
            outputs_tensor_keys = set(
                saved_model_loader.structured_outputs.keys()
            ).difference(exclude_outputs)
    ...
    def __init__(
        ...
        self._exclude_outputs = exclude_outputs if exclude_outputs is not None else []
        ...
        for passthrough_key, column_index in zip(
            ...
            if column_index >= 0:
                ...
                assert pa.types.is_list(
                    passthrough_data_column.type
                ) or pa.types.is_large_list(passthrough_data_column.type)
    ...
    def _handle_batch(self, batch):
        ...
        try:
            ...
            raise ValueError(
                f"""An error occurred while trying to apply the transformation: "{str(e)}".
          Batch instances: {batch},
          Fetching the values for the following Tensor keys: {self._graph_state.outputs_tensor_keys}."""
            ) from e
    ...
    def _make_graph_state(self, saved_model_dir):
        ...
        if self._use_tf_compat_v1:
            result = self._GraphStateCompatV1(
                saved_model_dir,
            ...
            result = self._GraphStateV2(
                saved_model_dir, self._get_input_tensor_names(), self._exclude_outputs
            )
...
def _maybe_slice_large_record_batch(
    ...
    if record_batch.nbytes > _MAX_TRANSFORMED_BATCH_BYTES_SIZE:
        ...
        yield from _maybe_slice_large_record_batch(record_batch.slice(offset=mid_point))
    else:
        yield record_batch
        ...
        if len(data) not in (batch_size, 1):
            ...
            if len(data_set) == 1:
                ...
                raise ValueError(
                    ...
                    f"are different ({len(data)} vs. {batch_size})"
                )
...
class _CreateTensorBindingsImpl(beam.PTransform):
    ...
    def expand(self, inputs):
        ...
        return pcoll | "ToTensorBinding" >> beam.Map(
            _TensorBinding, self._tensor_name, self._dtype_enum, self._is_asset_file
        )
    ...
    for tensor_binding in tensor_bindings:
        ...
        if graph is not None and tensor_binding.is_asset_filepath:
            graph.add_to_collection(
                tf.compat.v1.GraphKeys.ASSET_FILEPATHS, replacement_tensor
            )
        tensor_replacement_map[tensor_binding.tensor_name] = replacement_tensor
    return tensor_replacement_map


def _replace_tensors_with_constant_values(
    saved_model_dir, base_temp_dir, *tensor_bindings
):
    ...
    """
    with tf.compat.v1.Graph().as_default() as graph:
        tensor_replacement_map = _get_tensor_replacement_map(graph, *tensor_bindings)
...
@beam_common.register_ptransform(
    beam_nodes.CreateSavedModel, tags={beam_common.EnvironmentTags.TF_COMPAT_V1}
)
...
class _CreateSavedModelImpl(beam.PTransform):
    ...
    def expand(self, inputs):
        unbound_saved_model_dir = beam_common.get_unique_temp_path(self._base_temp_dir)
        ...
        return (
            inputs
            | "BindTensors"
            >> _BindTensors(self._base_temp_dir, unbound_saved_model_dir)
            | "Count" >> beam_common.IncrementCounter(_CREATE_SAVED_MODEL_COUNTER_NAME)
        )


def _create_v2_saved_model(
    tensor_replacement_map,
    base_temp_dir,
    preprocessing_fn,
    input_signature,
...
class _ApplySavedModelImpl(beam.PTransform):
    ...
    def expand(self, inputs):
        ...
        result = input_values_pcol | "ApplySavedModel" >> beam.ParDo(
            _RunMetaGraphDoFn(
                self._tf_config,
...
class _ExtractFromDictImpl(beam.PTransform):
    ...
    def expand(self, inputs):
        ...
        def extract_keys(input_dict, keys):
            return (
                tuple(input_dict[k] for k in keys)
                if isinstance(keys, tuple)
                else input_dict[keys]
            )
...
def _infer_metadata_from_saved_model(
    saved_model_dir: str, use_tf_compat_v1: bool
...
def _infer_metadata_from_saved_model_v1(
    ...
    with tf.compat.v1.Graph().as_default() as graph:
        with tf.compat.v1.Session(graph=graph) as session:
            _, outputs = saved_transform_io.partially_apply_saved_transform_internal(
                saved_model_dir, {}
            )
...
class _InstrumentAPI(beam.PTransform):
    ...
    def __init__(self, tf_graph, force_tf_compat_v1, use_tf_compat_v1):
        def _get_counter_from_graph_collection(collection_name):
            ...
            if len(collection) > 1:
                raise ValueError(
                    f"Expected TF graph collection '{collection_name}' to contain at most one element. "
                    f"Encountered {len(collection)}."
                )
    ...
    def expand(self, pipeline):
        def _make_and_increment_counters(
            unused_element,
            analyzer_counter,
            mapper_counter,
            force_tf_compat_v1,
            ...
            beam.metrics.Metrics.counter(
                beam_common.METRICS_NAMESPACE, "requested_tf_compat_v1"
            ).inc(int(force_tf_compat_v1))
            beam.metrics.Metrics.counter(
                beam_common.METRICS_NAMESPACE, "running_tf_compat_v1"
            ).inc(int(use_tf_compat_v1))
            for counter_prefix, counter in (
                ("tft_analyzer_{}", analyzer_counter),
                ("tft_mapper_{}", mapper_counter),
            ):
                for name, count in counter.items():
                    beam.metrics.Metrics.counter(
                        beam_common.METRICS_NAMESPACE, counter_prefix.format(name)
                    ).inc(count)
        ...
        _ = (
            ...
            | "CountAPIUse"
            >> beam.Map(
                _make_and_increment_counters,
                self._analyzer_use_counter,
...
class _InstanceDictInputToTFXIOInput(beam.PTransform):
    ...
    def expand(self, instance_dict_pcoll):
        return (
            ...
            >> beam.Map(example_proto_coder.ExampleProtoCoder(self._schema).encode)
            | "TfExampleToRecordBatch"
            >> self._tfxio.BeamSource(batch_size=self._desired_batch_size)
        )
...
def _make_output_cache(
    ...
    return {
        dataset_key: analyzer_cache.DatasetCache(cache, dataset_metrics[dataset_key])
...
class _AnalyzeDatasetCommon(beam.PTransform):
    ...
    def _extract_input_pvalues(self, dataset):
        ...
        assert (
            self.pipeline is not None or pvalues
        ), "If there is no data, a pipeline must be provided"
    ...
    def expand(self, dataset):
        ...
        if isinstance(input_metadata, dataset_metadata.DatasetMetadata):
            if Context.get_passthrough_keys():
                raise ValueError(
                    f"passthrough_keys is set to {Context.get_passthrough_keys()} but it is not supported"
        ...
        graph, structured_inputs, structured_outputs = (
            impl_helper.trace_preprocessing_function(
                self._preprocessing_fn, specs, self._use_tf_compat_v1, base_temp_dir
            )
        )
        ...
        pipeline = (
            ...
            or (
                flattened_pcoll
                or next(v for v in input_values_pcoll_dict.values() if v is not None)
            ).pipeline
        )
        ...
        _ = pipeline | "InstrumentAPI" >> _InstrumentAPI(
            graph, Context._get_force_tf_compat_v1(), self._use_tf_compat_v1
        ...
        if flattened_pcoll is not None:
            ...
            for idx, key in enumerate(sorted(input_values_pcoll_dict.keys())):
                ...
                if input_value is not None:
                    dataset_metrics[key] = (
                        input_value
                        | f"GetRecordBatchSize[{infix}]"
                        >> beam.Map(lambda rb: rb.nbytes)
                        | f"SumTotalBytes[{infix}]" >> beam.CombineGlobally(sum)
                        | f"ConstructMetadata[{infix}]"
                        >> beam.Map(analyzer_cache.DatasetCacheMetadata)
                    )
                    _ = (
                        input_value
        ...
        analyzers_fingerprint = (
            graph_tools.get_analyzers_fingerprint(graph, structured_inputs)
            if not self._use_tf_compat_v1
            else None
        )
        ...
        (transform_fn_future, cache_value_nodes, detached_sideeffect_leafs) = (
        ...
        output_cache_pcoll_dict = _make_output_cache(
            cache_value_nodes, traverser, dataset_metrics
        )
        ...
        full_metadata = beam_metadata_io.BeamDatasetMetadata(
            dataset_metadata.DatasetMetadata(schema=schema),
            deferred_metadata,
            asset_map,
        )
...
@beam.typehints.with_input_types(Union[common_types.InstanceDictType, pa.RecordBatch])
...
class TransformDataset(beam.PTransform):
    ...
    def expand(self, dataset_and_transform_fn):
        ...
        if isinstance(input_metadata, dataset_metadata.DatasetMetadata):
            if Context.get_passthrough_keys():
                raise ValueError(
                    f"passthrough_keys is set to {Context.get_passthrough_keys()} but it is not "
