    ...
    def expand(self, inputs):
        ...
        if (
            ...
            raise ValueError(
                ...
                f"None, got {self._coverage_frequency_threshold}."
            )
        (pcoll,) = inputs
...
class _VocabularyOrderAndWriteImpl(beam.PTransform):
    ...
    def expand(self, inputs):
        ...
        if self._file_format == "text":
            ...
            vocabulary_file = f"{vocabulary_file}.tfrecord.gz"
...
def _calculate_mutual_information_for_feature_value(
    ...
    if round(x_i) > round(n):
        raise ValueError(
            f"Frequency of token {feature_value} higher than number of records {x_i} > {n}"
...
def _split_inputs_by_key(batch_values):
    ...
    for arg_index, arg_values in enumerate(batch_values[1:]):
        if arg_values.ndim < 1:
            raise ValueError(
                f"Argument {arg_index} for CombinePerKey should have rank >=1, "
                f"got shape {arg_values.shape}"
            )
        if arg_values.shape[0] != keys.shape[0]:
            raise ValueError(
                f"Argument {arg_index} had shape {arg_values.shape} whose first dimension was not equal to the "
                f"size of the keys vector ({keys.shape[0]})"
            )
...
def _merge_outputs_by_key(keys_and_outputs, outputs_dtype):
    ...
    if len(outputs) != num_outputs:
        raise ValueError(
            f"Analyzer has {num_outputs} outputs but its implementation produced {len(outputs)} "
            "values"
        )
