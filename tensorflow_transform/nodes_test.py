...
class _Concat(collections.namedtuple('_Concat', []), nodes.OperationDef):
  pass


class _Swap(collections.namedtuple('_Swap', []), nodes.OperationDef):
...
class _Constant(collections.namedtuple('_Constant', ['value']),
                nodes.OperationDef):
  pass


class _Identity(collections.namedtuple('_Identity', ['name']),
                nodes.OperationDef):
  pass
...
class NodesTest(test_case.TransformTestCase):
  ...
  def testApplyOperationWithKwarg(self):
    ...
    self.assertEqual(op.operation_def, _Constant('a'))
  ...
  def testOperationNodeWithBadInput(self):
    ...
    with self.assertRaisesRegexp(
      ...
      nodes.OperationNode(_Concat(), (a, 'not a value_node'))

  def testOperationNodeWithBadInputs(self):
    with self.assertRaisesRegexp(
        TypeError, 'inputs must be a tuple, got'):
      nodes.OperationNode(_Concat(), 'not a tuple')
