...
def _ragged_tensor_representation_from_feature_spec(
    spec: tf.io.RaggedFeature, name: str, domains: Dict[str, common_types.DomainType]
) -> Tuple[
    schema_pb2.Feature, List[schema_pb2.Feature], schema_pb2.TensorRepresentation
]:
    ...
    for partition in spec.partitions:
        if isinstance(
            ...
            partition, tf.io.RaggedFeature.RowLengths
        ):  # pytype: disable=attribute-error
            ragged_tensor.partition.append(
                schema_pb2.TensorRepresentation.RaggedTensor.Partition(
    ...
    return (
        value_feature,
        partitions_features,
        schema_pb2.TensorRepresentation(ragged_tensor=ragged_tensor),
    )
...
def _sparse_feature_from_feature_spec(spec, name, domains):
    ...
    if isinstance(spec.index_key, list):
        ...
        spec_size = [
            s.value if isinstance(s, tf.compat.v1.Dimension) else s for s in spec.size
        ]
        ...
        index_feature = [
            schema_pb2.Feature(name=key, type=schema_pb2.INT, int_domain=int_domain)
            for (key, int_domain) in zip(spec.index_key, int_domains)
        ]
        index_feature_ref = [
            schema_pb2.SparseFeature.IndexFeature(name=key) for key in spec.index_key
        ]
    else:
        ...
        index_feature_ref = [schema_pb2.SparseFeature.IndexFeature(name=spec.index_key)]
...
def _feature_from_feature_spec(spec, name, domains):
    ...
    if isinstance(spec, tf.io.FixedLenFeature):
        if spec.default_value is not None:
            raise ValueError(
                f'feature "{name}" had default_value {spec.default_value}, but FixedLenFeature must have '
                "default_value=None"
            )
        ...
        raise TypeError(
            f'Spec for feature "{name}" was {spec} of type {type(spec)}, expected a '
            "FixedLenFeature, VarLenFeature or SparseFeature"
        )
...
def _standardize_default_value(spec: tf.io.FixedLenFeature) -> tf.io.FixedLenFeature:
...
def _get_string_domains(
    schema: schema_pb2.Schema,
) -> Dict[str, schema_pb2.StringDomain]:
...
def pop_ragged_source_columns(
    ...
    for column_path in source_columns[1:]:
        ...
        try:
            ...
            raise ValueError(
                missing_column_error_format.format(name, column_path.steps()[0])
            )
...
def _ragged_tensor_representation_as_feature_spec(
    ...
    value_feature = pop_ragged_source_columns(
    ...
    for name, spec in sorted(feature_spec.items()):
        if isinstance(spec, tf.io.FixedLenFeature):
            ...
            if len(spec.shape) == 0:  # pylint: disable=g-explicit-length-test
                ...
                raise ValueError(
                    f'When inferring legacy schema from feature spec, feature "{name}" had '
                    f"shape {spec.shape}, but FixedLenFeature must have shape [] or [k] where "
                    "k > 1."
                )
            ...
            if spec.default_value is None:
                ...
                raise ValueError(
                    f'When inferring legacy schema from feature spec, feature "{name}" had '
                    f"default_value {spec.default_value}, but FixedLenFeature must have "
                    f"default_value=None or {expected_default_value}"
                )
            ...
            raise TypeError(
                ...
                f'"{name}" was {spec} of type {type(spec)}, expected a FixedLenFeature or '
                "VarLenFeature "
            )
