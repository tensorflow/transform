...
from tensorflow_transform import graph_tools, test_case
...
def _create_graph_with_y_sparse_function_of_x_sparse():
    x = tf.compat.v1.sparse_placeholder(tf.int64)
    y = tf.SparseTensor(
        indices=x.indices, values=x.values + 1, dense_shape=x.dense_shape
    )
    return {
        "x": x,
        "y": y,
...
def _create_graph_with_y_function_of_x_and_untracked_table():
    ...
    y = table.lookup(x)
    del tf.compat.v1.get_collection_ref(tf.compat.v1.GraphKeys.TABLE_INITIALIZERS)[:]
    return {"filename": filename, "x": x, "y": y}
...
def _create_graph_with_table_initialized_by_table_output():
    ...
    keys = ["a", "b", "c"]
    tensor_keys = tf.as_string(table1.lookup(tf.constant(keys, tf.string)))
...
def _create_graph_with_y_function_of_x_with_tf_while():
    ...
    @tf.function(
        input_signature=[tf.TensorSpec([], tf.int32), tf.TensorSpec([], tf.int64)]
    )
    ...
    def __eq__(self, other):
        if not isinstance(other, self.expected_class):
            tf.compat.v1.logging.error(
                "Types do not match, got: %s, expected: %s",
                type(other),
                self.expected_class,
            )
            return False
        ...
        if self.expected_fields_values != self.expected_fields(
            other
        ) and future_expected_fields != self.expected_fields(other):
            tf.compat.v1.logging.error(
                "Fields do not match: %s != %s",
...
class _TensorMatcher(_Matcher, tfx_namedtuple.namedtuple("_TensorMatcher", ["name"])):
    __slots__ = ()
...
class GraphToolsTest(test_case.TransformTestCase):
    @test_case.named_parameters(
        ...
        dict(
            ...
            feeds=[],
            replaced_tensors_ready={"x": False, "x2": True, "x_unused": True},
            should_be_ready={"y": False, "z": True},
            num_ready_table_initializers=0,
        ),
        dict(
        ...
        dict(
            ...
            feeds=[],
            replaced_tensors_ready={"x": False, "filename": False},
            should_be_ready={"y": False},
            num_ready_table_initializers=0,
        ),
        dict(
            ...
            create_graph_fn=_create_graph_with_y_function_of_x_and_table,
            feeds=[],
            replaced_tensors_ready={"x": False, "filename": True},
            should_be_ready={"y": False},
        ...
        dict(
            ...
            create_graph_fn=_create_graph_with_y_function_of_x_and_table,
            feeds=[],
            replaced_tensors_ready={"x": True, "filename": False},
            should_be_ready={"y": False},
            num_ready_table_initializers=0,
        ),
        dict(
    ...
    def testDetermineReadyTensorsAndTableInitializers(
        ...
        self.assertEqual(
            len(graph_analyzer.ready_table_initializers), num_ready_table_initializers
        )
    ...
    def testInitializableGraphAnalyzerConstructorRaises(
        self, create_graph_fn, feeds, replaced_tensors_ready, error_msg_regex
    ):
        """Test determine_ready_tensors_and_table_initializers.
    ...
    def testInitializableGraphAnalyzerReadyToRunRaises(
        self, create_graph_fn, feeds, replaced_tensors_ready, fetch, error_msg_regex
    ):
        """Test determine_ready_tensors_and_table_initializers.
    ...
    def testGetDependentInputs(
        self, create_graph_fn, feeds, fetches, expected_dependent_inputs
    ):
        with tf.compat.v1.Graph().as_default() as graph:
            tensors = create_graph_fn()
        got = graph_tools.get_dependent_inputs(
            graph, {x: tensors[x] for x in feeds}, {y: tensors[y] for y in fetches}
        )
...
class GraphToolsTestUniquePath(test_case.TransformTestCase):
        ...
        dict(
            ...
            expected_calls_dict={
                ...
                "y": [
                    ...
                    mock.call(_TensorMatcher("add/y:0"), parents=["add/y"]),
                    mock.call("x$tensor"),
                    mock.call(_OpMatcher("add"), parents=["x$tensor", "add/y:0"]),
                    mock.call(_TensorMatcher("add:0"), parents=["add"]),
                ],
            },
        ),
        dict(
            ...
            create_graph_fn=_create_graph_with_tf_function,
            feeds=["x", "y"],
            replaced_tensors_ready={"x": False, "y": False},
            expected_calls_dict={
                "x": [
                    mock.call("x$tensor"),
                ],
                ...
                "z": [
                    ...
                    mock.call(_TensorMatcher("mul/y:0"), parents=["mul/y"]),
                    mock.call("FuncGraphInput[0]"),
                    mock.call(
                        _OpMatcher("mul"), parents=["FuncGraphInput[0]", "mul/y:0"]
                    ),
                    mock.call(_TensorMatcher("mul:0"), parents=["mul"]),
                    mock.call(_OpMatcher("Identity"), parents=["mul:0"]),
                    mock.call(_TensorMatcher("Identity:0"), parents=["Identity"]),
                    ...
                    mock.call(_TensorMatcher("add:0"), parents=["add"]),
                    mock.call(_OpMatcher("Identity_1"), parents=["add:0"]),
                    mock.call(_TensorMatcher("Identity_1:0"), parents=["Identity_1"]),
                    mock.call(_OpMatcher("mul_1/y"), parents=[]),
                    mock.call(_TensorMatcher("mul_1/y:0"), parents=["mul_1/y"]),
                    mock.call(
                        _OpMatcher("mul_1"), parents=["FuncGraphInput[1]", "mul_1/y:0"]
                    ),
                    mock.call(_TensorMatcher("mul_1:0"), parents=["mul_1"]),
                    mock.call(_OpMatcher("Identity_2"), parents=["mul_1:0"]),
                    mock.call(_TensorMatcher("Identity_2:0"), parents=["Identity_2"]),
                    mock.call(
                    ...
                    mock.call(
                        _TensorMatcher("PartitionedCall:2"), parents=["PartitionedCall"]
                    ),
                    mock.call(
                        _TensorMatcher("PartitionedCall:1"), parents=["PartitionedCall"]
                    ),
                    mock.call(
                        _TensorMatcher("PartitionedCall:0"), parents=["PartitionedCall"]
                    ),
                    mock.call(
                        _OpMatcher("add"),
                        parents=["PartitionedCall:0", "PartitionedCall:1"],
                    ),
                    mock.call(_TensorMatcher("add:0"), parents=["add"]),
                    mock.call(
                        _OpMatcher("add_1"), parents=["add:0", "PartitionedCall:2"]
                    ),
                    mock.call(_TensorMatcher("add_1:0"), parents=["add_1"]),
                ],
            },
        ),
        dict(
            ...
            expected_calls_dict={
                ...
                "y": [
                    ...
                    mock.call(_TensorMatcher("truediv/y:0"), parents=["truediv/y"]),
                    mock.call(_OpMatcher("truediv/Cast_1"), parents=["truediv/y:0"]),
                    mock.call(
                        _TensorMatcher("truediv/Cast_1:0"), parents=["truediv/Cast_1"]
                    ),
                    mock.call("x$tensor"),
                    mock.call(_OpMatcher("mul/y"), parents=[]),
                    mock.call(_TensorMatcher("mul/y:0"), parents=["mul/y"]),
                    ...
                    mock.call("FuncGraphInput[0]"),
                    mock.call(
                        _OpMatcher("add"), parents=["FuncGraphInput[0]", "add/y:0"]
                    ),
                    mock.call(_TensorMatcher("add:0"), parents=["add"]),
                    mock.call(_OpMatcher("Identity"), parents=["add:0"]),
                    mock.call(_TensorMatcher("Identity:0"), parents=["Identity"]),
                    mock.call(
                        _OpMatcher("PartitionedCall"),
                        parents=["FuncGraphInput[0]", "Identity:0"],
                    ),
                    mock.call(
                        _TensorMatcher("PartitionedCall:0"), parents=["PartitionedCall"]
                    ),
                    mock.call(
                        _OpMatcher("mul"), parents=["PartitionedCall:0", "mul/y:0"]
                    ),
                    mock.call(_TensorMatcher("mul:0"), parents=["mul"]),
                    mock.call(_OpMatcher("Identity"), parents=["mul:0"]),
                    mock.call(_TensorMatcher("Identity:0"), parents=["Identity"]),
                    mock.call(
                        _OpMatcher("PartitionedCall"),
                        parents=["x$tensor", "Identity:0"],
                    ),
                    mock.call(
                        _TensorMatcher("PartitionedCall:0"), parents=["PartitionedCall"]
                    ),
                    mock.call(
                        _OpMatcher("truediv/Cast"), parents=["PartitionedCall:0"]
                    ),
                    mock.call(
                        _TensorMatcher("truediv/Cast:0"), parents=["truediv/Cast"]
                    ),
                    mock.call(
                        _OpMatcher("truediv"),
                        parents=["truediv/Cast:0", "truediv/Cast_1:0"],
                    ),
                    mock.call(_TensorMatcher("truediv:0"), parents=["truediv"]),
                ],
            },
        ),
        dict(
            ...
            expected_calls_dict={
                "y": [
                    ...
                    mock.call(_TensorMatcher("range/delta:0"), parents=["range/delta"]),
                    mock.call("x$composite_tensor_2"),
                    mock.call(_OpMatcher("Rank"), parents=["x$composite_tensor_2"]),
                    mock.call(_TensorMatcher("Rank:0"), parents=["Rank"]),
                    mock.call(_OpMatcher("range/start"), parents=[]),
                    mock.call(_TensorMatcher("range/start:0"), parents=["range/start"]),
                    mock.call(
                        _OpMatcher("range"),
                        parents=["range/start:0", "Rank:0", "range/delta:0"],
                    ),
                    mock.call(_TensorMatcher("range:0"), parents=["range"]),
                    ...
                    mock.call(
                        ...
                        parents=[
                            ...
                            "x$composite_tensor_1",
                            "x$composite_tensor_2",
                            "range:0",
                        ],
                    ),
                    mock.call(
                        _TensorMatcher("SparseReduceSum:0"), parents=["SparseReduceSum"]
                    ),
                    mock.call(
                        _OpMatcher("add"), parents=["SparseReduceSum:0", "add/y:0"]
                    ),
                    mock.call(_TensorMatcher("add:0"), parents=["add"]),
                ]
            },
        ...
        dict(
            ...
            expected_calls_dict={
                "z": [
                    ...
                    mock.call(_TensorMatcher("ones/Const:0"), parents=["ones/Const"]),
                    mock.call("x$composite_tensor_2"),
                    mock.call(
                        _OpMatcher("ones"),
                        parents=["x$composite_tensor_2", "ones/Const:0"],
                    ),
                    mock.call(_TensorMatcher("ones:0"), parents=["ones"]),
                    mock.call(_OpMatcher("add/y"), parents=[]),
                    mock.call(_TensorMatcher("add/y:0"), parents=["add/y"]),
                    mock.call("x$composite_tensor_1"),
                    mock.call(
                        _OpMatcher("add"), parents=["x$composite_tensor_1", "add/y:0"]
                    ),
                    mock.call(_TensorMatcher("add:0"), parents=["add"]),
                    mock.call("x$composite_tensor_0"),
                    mock.call(
                ...
                "z": [
                    ...
                    mock.call(
                        ...
                        parents=[
                            ...
                            "RaggedToSparse/RaggedTensorToSparse:2",
                            "range:0",
                        ],
                    ),
                    mock.call(
        ...
        dict(
            ...
            expected_calls_dict={
                "y": [
                    ...
                    mock.call(_TensorMatcher("Less/y:0"), parents=["Less/y"]),
                    mock.call("FuncGraphInput[0]"),
                    mock.call(
                        _OpMatcher("Less"), parents=["FuncGraphInput[0]", "Less/y:0"]
                    ),
                    mock.call(_TensorMatcher("Less:0"), parents=["Less"]),
                    mock.call(_OpMatcher("Identity"), parents=["Less:0"]),
                    mock.call(_TensorMatcher("Identity:0"), parents=["Identity"]),
                    mock.call(_OpMatcher("Add/y"), parents=[]),
                    mock.call(_TensorMatcher("Add/y:0"), parents=["Add/y"]),
                    mock.call("FuncGraphInput[0]"),
                    ...
                    mock.call(_TensorMatcher("Add_1/y:0"), parents=["Add_1/y"]),
                    mock.call("FuncGraphInput[1]"),
                    mock.call(
                        _OpMatcher("Add_1"), parents=["FuncGraphInput[1]", "Add_1/y:0"]
                    ),
                    mock.call(_TensorMatcher("Add_1:0"), parents=["Add_1"]),
                    mock.call(_OpMatcher("Identity_1"), parents=["Add_1:0"]),
                    mock.call(_TensorMatcher("Identity_1:0"), parents=["Identity_1"]),
                    mock.call(
                        _OpMatcher("While"),
                        parents=[
                    ...
                    mock.call(_TensorMatcher("While:1"), parents=["While"]),
                ],
            },
        ),
        dict(
            ...
            expected_calls_dict={
                "y": [
                    ...
                    mock.call(_TensorMatcher("Identity:0"), parents=["Identity"]),
                    mock.call("FuncGraphInput[1]"),
                    mock.call(_OpMatcher("Identity_1"), parents=["FuncGraphInput[1]"]),
                    mock.call(_TensorMatcher("Identity_1:0"), parents=["Identity_1"]),
                    mock.call(_OpMatcher("mul/y"), parents=[]),
                    mock.call(_TensorMatcher("mul/y:0"), parents=["mul/y"]),
                    mock.call("FuncGraphInput[2]"),
                    mock.call(
                        _OpMatcher("mul"), parents=["FuncGraphInput[2]", "mul/y:0"]
                    ),
                    mock.call(_TensorMatcher("mul:0"), parents=["mul"]),
                    mock.call(_OpMatcher("Identity_2"), parents=["mul:0"]),
                    mock.call(_TensorMatcher("Identity_2:0"), parents=["Identity_2"]),
                    mock.call(
                        _OpMatcher("while"),
                        parents=[
                    ...
                    mock.call(_TensorMatcher("while:2"), parents=["while"]),
                    mock.call(_OpMatcher("Identity"), parents=["while:2"]),
                    mock.call(_TensorMatcher("Identity:0"), parents=["Identity"]),
                    mock.call(
                        _OpMatcher("PartitionedCall"),
                        parents=["x$tensor", "Identity:0"],
        ...
        dict(
            ...
            expected_calls_dict={
                ...
                "y": [
                    ...
                    mock.call(_TensorMatcher("hash_table:0"), parents=["hash_table"]),
                    mock.call(
        ...
        dict(
            ...
            expected_calls_dict={
                ...
                "z": [
                    mock.call("y$tensor"),
                    mock.call("x$tensor"),
                    mock.call(_OpMatcher("Equal"), parents=["x$tensor", "y$tensor"]),
                    mock.call(_TensorMatcher("Equal:0"), parents=["Equal"]),
                    mock.call(
                        _OpMatcher("Assert"),
                        parents=["Equal:0", "x$tensor", "y$tensor"],
                    ),
                    mock.call(
                        _OpMatcher("control_dependency"), parents=["x$tensor", "Assert"]
                    ),
                    mock.call(
                        _TensorMatcher("control_dependency:0"),
                        parents=["control_dependency"],
                    ),
                ],
            },
        ),
    ...
    def testGetUniquePath(
        self, create_graph_fn, feeds, replaced_tensors_ready, expected_calls_dict
    ):
        with tf.compat.v1.Graph().as_default() as graph:
            tensors = create_graph_fn()
        replaced_tensors_ready = [
        ...
        for name in expected_calls_dict:
            ...
            def describe_path_fn(x, parents=None):
                ...
                actual_needed_matchers_to_pass.append(
                    "({}{}),".format(  # pylint: disable=cell-var-from-loop
            ...
            try:
                ...
                self.assertEqual(
                    ...
                    len(expected_calls_dict[name]),
                    "Number of expected calls != number of actual calls for {}: {}".format(
                        name, path_cb_mock.call_args_list
                    ),
                )
...
def _value_to_matcher(value, add_quotes=False):
    ...
    if isinstance(value, (str, bytes)):
        if add_quotes:
            ...
            return value
    raise ValueError("Cannot get a matcher for: {}, {}".format(type(value), value))
