...
from tensorflow_transform import gaussianization, test_case

_MEAN_SCALE_SCALAR_TEST = dict(
    testcase_name="_scalar",
    h_params=np.array([0.1, 0.2], np.float32),
    expected_outputs=[np.float32(0.05540865005575452), np.float32(0.6932738015273474)],
)
...
_MEAN_SCALE_ND_TEST = dict(
    ...
    expected_outputs=[
        np.array(
            [
                [[0.0], [0.8865384], [0.19947124]],
                [[-0.75989], [-1.4960338], [-3.5904799]],
            ],
            np.float32,
        ),
        np.array(
            [
                [[0.5641896], [1.4878997], [1.4943897]],
                [[1.6034254], [2.1926064], [4.085859]],
            ],
            np.float32,
        ),
    ],
...
_L_SKEWNESS_KURTOSIS_SCALAR_TEST = dict(
    testcase_name="_scalar",
    h_params=np.array([0.1, 0.2], np.float32),
    expected_outputs=[np.float32(0.05989154619056726), np.float32(0.21460719619685548)],
)
...
_L_SKEWNESS_KURTOSIS_ND_TEST = dict(
    ...
    expected_outputs=[
        np.array(
            [
                [[0.0], [0.5209037], [0.11905935]],
                [[-0.4226278], [-0.6249933], [-0.833552]],
            ],
            np.float32,
        ),
        np.array(
            [
                [[0.12260159], [0.54675657], [0.5140212]],
                [[0.55600286], [0.66664696], [0.81815743]],
            ],
            np.float32,
        ),
    ],
...
_COMPUTE_TUKEY_H_PARAMS_PATOLOGICAL_TESTS = [
    dict(
        ...
        l_skewness_and_kurtosis=np.array([0.7, 0.5], np.float32),
        expected_output=np.array([0.0, 0.65736556], np.float32),
    ),
    dict(
        testcase_name="_pat_invalid_skewness",
        l_skewness_and_kurtosis=np.array([1.0, 0.5], np.float32),
        expected_output=np.array([0.0, 0.65736556], np.float32),
    ),
    dict(
        testcase_name="_pat_invalid_kurtosis",
        l_skewness_and_kurtosis=np.array([0.5, 1.5], np.float32),
        expected_output=np.array([00.9999859847861059, 0.9999950120303265], np.float32),
    ),
]
...
_INVERSE_TUKEY_HH_ND_TESTS = [
    dict(
        ...
        samples=np.array(_tukey_hh(np.linspace(-5.0, 5.0, 20), 1.0, 2.0), np.float32),
        hl=np.float32(1.0),
        hr=np.float32(2.0),
...
class GaussianizationTest(test_case.TransformTestCase):
    @test_case.named_parameters(_MEAN_SCALE_SCALAR_TEST, _MEAN_SCALE_ND_TEST)
    ...
    @test_case.named_parameters(
        _L_SKEWNESS_KURTOSIS_SCALAR_TEST, _L_SKEWNESS_KURTOSIS_ND_TEST
    )
    ...
    def test_compute_tukey_hh_params(self, l_skewness_and_kurtosis, expected_output):
