    ...
    for name, tensor in tensors.items():
        ...
        if isinstance(tensor, tf.SparseTensor):
            ...
            if shape.ndims in [None, 0]:
                raise ValueError(
                    f"Feature {name} ({tensor}) had invalid shape {shape} for FixedLenFeature: must "
            ...
            if is_evaluation_complete and any(
                ...
                raise ValueError(
                    f"Feature {name} ({tensor}) had invalid shape {shape} for FixedLenFeature: apart "
            ...
            raise TypeError(
                f"Expected a Tensor, SparseTensor, or RaggedTensor got {tensor} of type {type(tensor)} "
                f"for feature {name}"
            )
