...
class ValueNode:
    ...
    def __post_init__(self):
        ...
        if not (self.value_index >= 0 and self.value_index < num_outputs):
            raise ValueError(
                f"value_index was {self.value_index} but parent_operation had {num_outputs} outputs"
            )
...
class OperationNode:
    ...
    def __init__(self, operation_def, inputs):
        ...
        if not isinstance(inputs, tuple):
            raise TypeError(
                f"inputs must be a tuple, got {inputs} of type {type(inputs)}"
            )
...
class Traverser:
    ...
    def _visit_operation(self, operation: OperationNode):
        ...
        if not output_iterable or isinstance(output_values, (ValueNode, OperationDef)):
            raise ValueError(
                f"When running operation {operation.operation_def.label} expected visitor to return a tuple, got "
                f"{output_values} of type {type(output_values)}"
            )
        # DoOutputsTuple doesn't work with len().
        if hasattr(output_values, "__len__") and len(output_values) != len(outputs):
            raise ValueError(
                f"Operation {operation.operation_def} has {len(outputs)} outputs but visitor returned {len(output_values)} values: "
                f"{output_values}"
            )
...
class _PrintGraphVisitor(Visitor):
    ...
    def visit(self, operation_def, input_nodes) -> Tuple[pydot.Node, ...]:
        ...
        if num_outputs != 1:
            ports = "|".join(f"<{idx}>{idx}" for idx in range(num_outputs))
