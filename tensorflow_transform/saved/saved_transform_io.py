...
def _maybe_register_addon_ops():
    ...
    def _try_import(name):
        try:
            ...
            tf.compat.v1.logging.info(f"{name} is not available.")
            pass
...
def _partially_apply_saved_transform_impl(
    ...
    if unexpected_inputs:
        raise ValueError("Unexpected inputs " f"to transform: {unexpected_inputs}")
    ...
    if tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES in meta_graph_def.collection_def:
        ...
        if trainable_vars:
            raise ValueError(
                f"The SavedModel contained trainable variables {trainable_vars}.  Because this "
