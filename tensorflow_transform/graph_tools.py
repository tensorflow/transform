...
def _reraise_unexpected_error(func):
    ...
    def wrapper(self, tensor_or_op):
        ...
        try:
            ...
            raise ValueError(
                f"The tensor_or_op {tensor_or_op} depended on a placeholder ({e.tensor}) that was not "
                "in the input_signature.  This may have be caused by manually "
                f"adding a placeholder to the graph.{context}"
            ) from e
        except _UnexpectedTableError as e:
            if e.func_graph_name:
                raise ValueError(
                    f"The tensor_or_op {tensor_or_op} depended on an initializable table ({e.op}) that "
                    f"is part of a tf.function graph ({e.func_graph_name}), this is not supported. This"
                ...
                raise ValueError(
                    ...
                    "the collection tf.GraphKeys.TABLE_INITIALIZERS"
                ) from e
...
class InitializableGraphAnalyzer:
        ...
        for table_init_op_or_tensor in graph.get_collection(
            ...
            if isinstance(graph, tf_func_graph.FuncGraph):
                self._log_warning(
                    ...
                    f" graph context using  `tf.init_scope`.: {table_init_op_or_tensor.name}"
                )
        ...
        for name, tensor in input_signature.items():
            if isinstance(tensor, tf.Tensor):
                _set_unique_value_in_dict(
                    result, tensor, _SourceInfo(True, f"{name}$tensor")
                )
    ...
    def _get_table_init_op_source_info(
        ...
        if not table_init_op.inputs:
            raise ValueError(
                f"Table initializer {table_init_op} did not have expected number if inputs "
                "(expected >= 1 inputs, got 0)"
            )
        ...
        try:
            ...
            if e.func_graph_name:
                raise e
            raise ValueError(
                f"The table initializer {table_init_op} depended on a placeholder ({e.tensor}).  Note "
            ...
            if e.func_graph_name:
                raise e
            raise ValueError(
                f"The table initializer {table_init_op} depended on an initializable table ({e.op}). "
                "Note tables are initialized in one pass so a table initializer "
                "cannot depend on the output of an initializeable table"
            ) from e
    ...
    def get_dependent_inputs(self, tensor_or_op):
        ...
        if not isinstance(
            ...
            raise TypeError(
                f"Expected Tensor, SparseTensor, RaggedTensor or Operation got {tensor_or_op} of "
                f"type {type(tensor_or_op)}"
            )
