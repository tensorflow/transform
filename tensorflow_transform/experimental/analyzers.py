...
def ptransform_analyzer(
    ...
    if len(output_dtypes) != len(output_shapes):
        raise ValueError(
            f"output_dtypes ({output_dtypes}) and output_shapes ({output_shapes}) had different"
            " lengths"
        )
    if output_asset_default_values is not None:
        if len(output_asset_default_values) != len(output_dtypes):
            raise ValueError(
                f"output_dtypes ({output_dtypes}) and output_asset_default_values ({output_asset_default_values}) had "
                "different lengths"
            )
    ...
    *,  # Force passing optional parameters by keys.
    ...
    if top_k <= 0:
        ...
        raise ValueError(
            ...
            f"be more efficient. Maximum allowed top_k is {analyzers.LARGE_VOCAB_TOP_K}"
        )

    if file_format not in analyzers.ALLOWED_VOCABULARY_FILE_FORMATS:
        raise ValueError(
            f'"{file_format}" is not an accepted file_format. It should be one of: {analyzers.ALLOWED_VOCABULARY_FILE_FORMATS}'
        )
